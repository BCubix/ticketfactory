...
class DashboardController extends AdminController
{
    ...
    public function index(Request $request, ParamFetcher $paramFetcher): View
    {
        ...
        $validatedOrders = $this->em->getRepository(Order::class)->findValidatedOrders();

        $totalSales = 0;
        foreach ($validatedOrders as $validatedOrder) {
            $data = $validatedOrder->getOrderData();
            $cart = $data['cart'];

            $totalSales += $cart['total'];
        ...
        if (null == $tab) {
            $data = [
                'col1' => [
                    'onlineVisitors' => $NumberActivePeople,
                    'activeBaskets' => $NumberActiveBasket,
                    'recordedEvents' => $NumberRecordedEvent,
                    'newsCustomers' => $NumberClients,
                    'websiteHealth' => [
                        'score' => 3,
                        'nbErrors' => 8,
    ...
    #[Rest\Get('/clientVisit')]
    #[Rest\QueryParam(name: 'beginDate', default: null)]
    #[Rest\QueryParam(name: 'endDate', default: null)]
    #[Rest\View(serializerGroups: ['a_all'])]
    public function getClientVisits(Request $request, ParamFetcher $paramFetcher): View
    {
        $beginDate = $paramFetcher->get('beginDate');
        $endDate = $paramFetcher->get('endDate');
