...
export const imageFormatsListCrud = {
    ...
    listTitle: 'Liste des emplacements de média',
    filtersData: [
    ...
    tableList: [
        ...
        { name: 'width', label: 'Largeur', width: '15%', renderFunction: (item) => <Typography>{item?.imageToCrop ? item?.width : '-'}</Typography>, sortable: true },
        { name: 'height', label: 'Hauteur', width: '15%', renderFunction: (item) => <Typography>{item?.imageToCrop ? item?.height : '-'}</Typography>, sortable: true },
    ],
    ...
    deleteComponent: ({ deleteDialog, setDeleteDialog, handleDelete }) => (
        <Component.DeleteDialog open={deleteDialog ? true : false} onCancel={() => setDeleteDialog(null)} onDelete={() => handleDelete(deleteDialog)}>
            <Box textAlign="center" py={3}>
                <Typography component="p">Êtes-vous sûr de vouloir supprimer cet Emplacement de média ?</Typography>
