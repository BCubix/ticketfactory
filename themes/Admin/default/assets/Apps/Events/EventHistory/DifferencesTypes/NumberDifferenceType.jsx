...
export const NumberDifferenceType = ({ previousVersion, actualVersion, nextVersion, basePreviousVersion, selectedHistory, name, label = '' }) => {
            ...
            <Grid container spacing={4}>
                        ...
                        {displayDifferences.isDifferentFromPrevious && (
                            <Box className="history_diff_display_old">
                                <Typography component="pre" style={{ margin: 0 }}>
                                    {previousVersionValue !== undefined ? previousVersionValue : 'Non renseigné'}
                                </Typography>
                            </Box>
                        )}
                    </Grid>
                ...
                <Grid item xs={Boolean(previousVersion) || Boolean(basePreviousVersion) ? 4 : 6}>
                    <Box className="history_diff_actual">
                        <Typography component="pre" style={{ margin: 0 }}>
                            {actualVersionValue !== undefined ? actualVersionValue : 'Non renseigné'}
                        </Typography>
                    </Box>
                </Grid>
                ...
                <Grid item xs={Boolean(previousVersion) || Boolean(basePreviousVersion) ? 4 : 6}>
                    {displayDifferences.isDifferentFromNext && (
                        <Box className="history_diff_display_new">
                            <Typography component="pre" style={{ margin: 0 }}>
                                {nextVersionValue !== undefined ? nextVersionValue : 'Non renseigné'}
                            </Typography>
                        </Box>
                    )}
                </Grid>
