...
const MonthModeView = (props) => {
  ...
  const selectedData = useMemo(() => {
    ...
    const filteredData = values.eventDate.filter((event) => {
      let parsedDate = null;
      try {
  ...
  const handleAddDateClick = () => {
    const index = values.eventDate.length;

    setDialogItemIndex(index);
    setFieldValue(`eventDate.${index}`, {
      eventDate: '',
      annotation: '',
      state: 'valid',
      reportDate: '',
  ...
  return (
    <Box>
            ...
            {selectedData.map((item, index) => {
              ...
              return (
                <SelectedTimeBox
                  ...
                  onClick={() => handleTimeClick(time, item)}
      ...
      <Dialog open={dialogOpen} onClose={handleCloseDialog}  sx={{
      '& .MuiDialog-paper': {
          overflow: 'visible',
        },
      }}>
        ...
        <DialogActions>
          ...
          />
          {!creatingItem && (
            <Button onClick={handleDeleteItem} color="error">Supprimer</Button>
          )}
